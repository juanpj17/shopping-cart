...
# GANANCIAS TOTALES
    def get_total_profit(self):
        statement = (
            select(
                func.sum(ProductModel.price * ProductCartModel.quantity).label("total_income"),
                func.sum((ProductModel.price - ProductModel.cost) * ProductCartModel.quantity).label("total_profit"),
            )
            .join(CartModel, CartModel.entity_id == ProductCartModel.cart_id)
            .join(OrderModel, OrderModel.cart_id == CartModel.entity_id)
            .join(ProductModel, ProductModel.entity_id == ProductCartModel.product_id)
    ...
    def get_total_profit_by_products(self):
        statement = (
            select(
                ProductModel.name,
                func.sum(ProductModel.price * ProductCartModel.quantity).label("total_income"),
