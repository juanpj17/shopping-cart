...
class GetTopProducts(Service[None, List[str, int]]):
    def __init__(self, report_repository: ReportsRepository):
        self.report_repository = report_repository
    
    def execute(self):
        report = self.report_repository.get_top_products()
        if not report: Result[List[str, int]].make_failure(error = ProductNotFoundError())

        return Result[List[str, int]].make_success(value = report)